...
feat.cust.global <- data.frame(cbind('customer_id'=as.character(cust$customer_id),
                                     ...
                                     'business_channel'=cust$mod_business_channel,
...
feat.cust.global$created_at_mon = as.POSIXlt(cusfeat.cust.globalt$created_at)$mon + 1
...
feat.cust.global$first_order_date_wday = as.POSIXlt(feat.cust.globalst$first_order_date)$wday + 1
...
feat.cust.global$days_lastorder_to_firstoder = (as.numeric(feat.cust.global$last_order_date) + 1 - as.numeric(feat.cust.global$first_order_date))/(60*60*24)
feat.cust.global$days_firstorder_to_created = (as.numeric(feat.cust.global$first_order_date) + 1- as.numeric(feat.cust.global$created_at))/(60*60*24)

head(feat.cust.global[which(feat.cust.global$days_firstorder_created==0),]$customer_id)
feat.cust.global[feat.cust.global$customer_id== 'u9038',]

head(feat.cust.global[which(feat.cust.global$days_lastorder_created==-1),]$customer_id)
feat.cust.global[feat.cust.global$customer_id== 'u256978',c('customer_id','created_at',"first_order_date",'last_order_date')]

# Check Plot
par(mfrow=c(2,2))
for(i in 21:23) {
  hist(feat.cust.global[,i], main=names(feat.cust.global)[i])
}

x$days_lastorder_to_created = sapply(feat.cust.global$days_lastorder_created, function(x) log(x))
x$days_lastorder_to_firstoder = sapply(feat.cust.global$days_lastorder_firstoder, function(x) log(x+1))
x$days_firstorder_to_created = sapply(feat.cust.global$days_firstorder_created, function(x) log(x+1))

########################################################
## ========== LITTLE COSMETIC MODIFICATION OF PROD TABLE
########################################################

# category, type, created_at, updated_at, product_lifecycle, telesales_only, manufacturer, daysperlens, promotional, 
# average_cost, pack_qty
# manufacturer :Coopervision,Bausch & Lomb,Ciba Vision,AMO, Johnson & Johnson
...
## note : category : "colours" in product type is actually monthly product -> still include in the analysis, since the number if transactino
## isn't that much
...
# prod[which(prod$category== 'Travel Packs'),]
# levels(prod$mod.category)

# refactoring days perlens
prod$mod.daysperlens <- prod$daysperlens
# levels(prod$daysperlens)
...
levels(prod$product_lifecycle)
levels(prod$mod.product_lifecycle) <- c('other','discountinued','other','regular')

# refactoring pack qty
prod$mod.pack_qty <- prod$pack_qty
# levels(prod$pack_qty)
levels(prod$mod.pack_qty) <- c('1','more_than10','2','more_than10','3_to6',
                                        'more_than10','3_to6','3_to6','more_than10','1')

## Featurize Average Cost
# install.packages('pastecs')
# library(pastecs)
# stat.desc(prod[,c("average_cost")])
# hist(prod$average_cost)
prod$average_cost = as.numeric(as.character(prod$average_cost))
prod$average_cost[which(is.na(prod$average_cost))] <- 0

# length(prod$average_cost[which(is.na(prod$average_cost))]) # 38 products have NULL price

# create bucket of price list
breaks = c(seq(0,2.5,0.5),seq(5,25,5))
tmp <- data.frame(matrix(ncol = (length(breaks)-1), nrow = nrow(prod)))
for (i in 1:(length(breaks)-1)){
  tmp[,i] <- as.numeric(prod$average_cost[i] & prod$average_cost < breaks[i+1])
}
colnames(tmp) = paste0('avg_cost_', 1:(length(breaks)-1))
prod <- cbind(prod, tmp)


write.csv(prod[,c(1,2)], 'data/keys/newid_name.key')

########################################################
## ========== ITEM FEATURE GLOBAL
########################################################
feat.item.global = data.frame(cbind('customer_id'=as.character(prod$product_id),
                                    'manufactur'=as.factor(prod$mod.manufacturer),
                                    'category' =as.factor(cust$mod.category),
...
# featurize manufactur
levels(feat.item.global$manufactur) <-   levels(prod$manufacturer)
...
# refactoring 
unique(prod$daysperlens) # categorize - done
unique(prod$promotional) # numerize - make it in item coupon global
unique(prod$telesales_only) # numerize - make it in item coupon global
unique(prod$product_lifecycle) # categorize - done
unique(prod$pack_qty) # categorize - done
unique(prod$average_cost) # bucket price - done
...
## investigation product (todolit 1 - 23 July 2016)
mod.trans <- read.csv('data/trans.csv')
head(mod.trans[which(mod.trans$product_id %in% x),c("customer_id",'order_no')])

x1 = key[which(key$order_no == '400271538'),]

x = prod[which(prod$category== 'Two Weeklies'),]$product_id
head(mod.trans[which(mod.trans$product_id %in% x),c("customer_id",'order_no','product_id')])

x1 = key[which(key$order_no == '400271538'),]
prod[which(prod$product_id== 'p163'),]
