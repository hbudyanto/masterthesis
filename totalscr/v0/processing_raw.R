...
cust$created_at = as.POSIXct(as.Date(cust$created_at[1:5],"%Y-%m-%d"))
cust$updated_at= as.POSIXct(as.Date(cust$updated_at[1:5],"%Y-%m-%d"))
...
R <- levels(as.factor(report2$customer_id))
customer_id <- unique(c(key.cust,C,D,R))
...
# drop those columns in trans
colnames(trans)[-c(7:8,11,13:15,17,20:28,35:36,41:42)]
...
prod$created_at = as.POSIXct(as.Date(prod$created_at[1:5],"%Y-%m-%d"))
prod$updated_at= as.POSIXct(as.Date(prod$updated_at[1:5],"%Y-%m-%d"))
...
colnames(prod) <- c("product_id","name","status","category","type","modality","created_at","updated_at","product_type","product_lifecycle",
                    "telesales_only","stocked_lens","average_cost","manufacturer","daysperlens","alt_daysperlens","condition",
                    "promotional_product","pack_qty")

write.csv(prod1, 'data/prod.csv', row.names = F, col.names = T, sep=',')
write.table(weblog, 'data/weblog.csv', row.names = F, col.names = T, sep = ',')


###################################################################################
unique(prod$category)
product[which(product$category=='Monthlies'),]

product[which(product$product_id==1088),]
x1 = transaction[which(transaction$product_id==1088),]
x2 = transaction[which(transaction$order_no==400271481),]
x3 = tmp[which(tmp$order_no==400271481),]

x3 = tmp[which(tmp$order_no==200113989),]

x3[-which(x3$categoryTwo.Weeklies!=0),]
...
tmp.key[tmp.key$order_no == '200111238',]
data[data$order_no == '200111238',]


#data = tmp.key[which(duplicated(tmp.key$order_no)),]
x = data[!duplicated(data),]
y = data[1:1000,]

x = data.frame(cbind(y, data.frame(model.matrix(~category-1, y))))
x[,c("product_id","category")] <- NULL
tmp.melt <- melt(x, id = c("order_date", "order_no"))
x1= dcast(tmp.melt, order_date + order_no ~ variable, sum)

x2 = x1[-which(x1$categoryTwo.Weeklies != 0 & x1$categoryMonthlies != 0),]

x1[which(x1$categoryTwo.Weeklies != 0 & x1$categoryMonthlies != 0),]
