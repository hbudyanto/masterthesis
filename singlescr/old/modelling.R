...
stopCluster(cl)
# release some memo
rm(tmp.data,tmp.matrix, tmp.holder)
...
purchaseTemp <- getPurchaseData(feature = 'category', numeric = 'actual_qty', holder = featCatNbProd, customer = id)
purchaseTemp <- getPurchaseData(feature = 'subcategory', numeric = 'actual_qty', holder = featSubNbProd, customer = id)
purchaseTemp <- getPurchaseData(feature = 'manufacturer', numeric = 'actual_qty', holder = featManNbProd, customer = id)
purchaseTemp <- getPurchaseData(feature = 'brand', numeric = 'actual_qty', holder = featBrandNbProd, customer = id)
purchaseTemp <- getPurchaseData(feature = 'subcat.lens.manu', numeric = 'actual_qty', holder = featSubLensmanuNbProd, customer = id)
purchaseTemp <- getPurchaseData(feature = 'lens.brand.manu', numeric = 'actual_qty', holder = featLensBrandManuNbProd, customer = id)
purchaseTemp <- getPurchaseData(feature = 'subcat.brand', numeric = 'actual_qty', holder = featSubBrandNbProd, customer = id)
purchaseTemp <- getPurchaseData(feature = 'lens.brand', numeric = 'actual_qty', holder = featLensBrandNbProd, customer = id)

purchaseTemp <- getPurchaseData(feature = 'category', numeric = 'global_line_total_exc_vat', holder = featCatTransSpent, customer = id)
purchaseTemp <- getPurchaseData(feature = 'subcategory', numeric = 'global_line_total_exc_vat', holder = featSubTransSpent, customer = id)
purchaseTemp <- getPurchaseData(feature = 'manufacturer', numeric = 'global_line_total_exc_vat', holder = featManTransSpent, customer = id)
purchaseTemp <- getPurchaseData(feature = 'brand', numeric = 'global_line_total_exc_vat', holder = featBrandTransSpent, customer = id)
purchaseTemp <- getPurchaseData(feature = 'subcat.lens.manu', numeric = 'global_line_total_exc_vat', holder = featSubLensmanuTransSpent, customer = id)
purchaseTemp <- getPurchaseData(feature = 'lens.brand.manu', numeric = 'global_line_total_exc_vat', holder = featLensBrandManuTransSpent, customer = id)
purchaseTemp <- getPurchaseData(feature = 'subcat.brand', numeric = 'global_line_total_exc_vat', holder = featSubBrandTransSpent, customer = id)
purchaseTemp <- getPurchaseData(feature = 'lens.brand', numeric = 'global_line_total_exc_vat', holder = featLensBrandTransSpent, customer = id)

purchaseTemp <- getPurchaseData(feature = 'category', numeric = 'apprx_discount', holder = featCatDiscReceived, customer = id)
purchaseTemp <- getPurchaseData(feature = 'subcategory', numeric = 'apprx_discount', holder = featSubDiscReceived, customer = id)
purchaseTemp <- getPurchaseData(feature = 'manufacturer', numeric = 'apprx_discount', holder = featManDiscReceived, customer = id)
purchaseTemp <- getPurchaseData(feature = 'brand', numeric = 'apprx_discount', holder = featBrandDiscReceived, customer = id)
purchaseTemp <- getPurchaseData(feature = 'subcat.lens.manu', numeric = 'apprx_discount', holder = featSubLensmanuDiscReceived, customer = id)
purchaseTemp <- getPurchaseData(feature = 'lens.brand.manu', numeric = 'apprx_discount', holder = featLensBrandManuDiscReceived, customer = id)
purchaseTemp <- getPurchaseData(feature = 'subcat.brand', numeric = 'apprx_discount', holder = featSubBrandDiscReceived, customer = id)
purchaseTemp <- getPurchaseData(feature = 'lens.brand', numeric = 'apprx_discount', holder = featLensBrandDiscReceived, customer = id)

purchaseTemp <- getPurchaseData(feature = 'category', numeric = 'qty', holder = featCatUniqueOrder, customer = id)
purchaseTemp <- getPurchaseData(feature = 'subcategory', numeric = 'qty', holder = featSubUniqueOrder, customer = id)
purchaseTemp <- getPurchaseData(feature = 'manufacturer', numeric = 'qty', holder = featManUniqueOrder, customer = id)
purchaseTemp <- getPurchaseData(feature = 'brand', numeric = 'qty', holder = featBrandUniqueOrder, customer = id)
purchaseTemp <- getPurchaseData(feature = 'subcat.lens.manu', numeric = 'qty', holder = featSubLensmanuUniqueOrder, customer = id)
purchaseTemp <- getPurchaseData(feature = 'lens.brand.manu', numeric = 'qty', holder = featLensBrandManuUniqueOrder, customer = id)
purchaseTemp <- getPurchaseData(feature = 'subcat.brand', numeric = 'qty', holder = featSubBrandUniqueOrder, customer = id)
purchaseTemp <- getPurchaseData(feature = 'lens.brand', numeric = 'qty', holder = featLensBrandUniqueOrder, customer = id)
...
# nsample = 10100; load(paste('features/features.cust.item.random/purchaseData',nsample,'.seed',123,'.rda',sep=''))
...
# save original files before data splitting
save(training, pre2015, testing, fullSet, reducedSet , file = paste('features/features.cust.item.random/preModelData',nsample,'.seed',123,'.rda',sep=''))
# nsample<- 10100; load(paste('features/features.cust.item.random/preModelData',nsample,'.seed',123,'.rda',sep=''))

trainingFin <- training[complete.cases(training),]
testingFin <- testing[complete.cases(testing),]

#############################################################################################
####### reformat target as a factor
obj <- which(colnames(trainingFin)== 'target')
trainingFin[,obj]= factor(trainingFin[,obj], labels = c('N','Y'))
testingFin[,obj]= factor(testingFin[,obj], labels = c('N','Y'))
...
set.seed(123)
strt<-Sys.time() # start time
xgbTree <- train(x = trainingFin[,reducedSet],
                 y = trainingFin$target,
                 method = "xgbTree",
                 metric = "ROC",
...
nnetFit <- train(x = trainingFin[,reducedSet], 
                 y = trainingFin$target,
                 method = "nnet",
                 metric = "ROC",
                 preProc = c("center", "scale"),
...
set.seed(123)
strt<-Sys.time() # start time
nnetFit2 <- train(x =  trainingFin[,reducedSet], 
                  y = trainingFin$target,
                  method = "nnet",
                  metric = "ROC",
...
set.seed(123)
strt<-Sys.time() # start time
nnetFit3 <- train(x = trainingFin[,reducedSet], 
                  y = trainingFin$target,
                  method = "avNNet",
                  metric = "ROC",
                  preProc = c("center", "scale"),
...
set.seed(123)
strt<-Sys.time() # start time
nnetFit4 <- train(x = trainingFin[,reducedSet], 
                  y = trainingFin$target,
                  method = "avNNet",
                  metric = "ROC",
                  preProc = c("center", "scale", "spatialSign"),
...
glmnGrid <- expand.grid(alpha = c(0,  .1, .4), 
                        lambda = seq(.01, .2, length = 25))

set.seed(123)
strt<-Sys.time() # start time
glmnFit <- train(x = trainingFin[,reducedSet], 
                 y = trainingFin$target,
                 method = "glmnet",
                 tuneGrid = glmnGrid,
                 preProc = c("center", "scale"),
...
sigmaRangeFull <- sigest(data.matrix(training[,reducedSet]))
...
set.seed(123)
strt<-Sys.time() # start time
svmRFitFull <- train(x = trainingFin[,reducedSet], 
                     y = trainingFin$target,
                     method = "svmRadial",
                     metric = "ROC",
                     preProc = c("center", "scale"),
...
svmPGrid <-  expand.grid(degree = 1:2,
                         scale = c(0.01, .005),
                         C = 2^(seq(-6, -2, length = 5)))
set.seed(123)
strt<-Sys.time() # start time
svmPFitFull <- train(x = trainingFin[,reducedSet], 
                     y = trainingFin$target,
                     method = "svmPoly",
                     metric = "ROC",
                     preProc = c("center", "scale"),
...
validation <- data.frame(orderYr = testingFin$orderYr, customer_id = testingFin$customer_id, product_id = testingFin$product_id, 
                         obs = testingFin$target)
validation$label <- ifelse(validation$obs == 1,
                           "Actual_outcome:Purchase", 
                           "Actual_outcome:Not_Purchase")

# Model : Extreme Gradient Boosting
validation$prob.xgbTree <- predict(xgbTree, testingFin[,reducedSet],type='prob')[,'Y']
validation$pred.xgbTree <- predict(xgbTree, testingFin[,reducedSet])

# Model : Neural Network (model1)
validation$prob.nnetFit <- predict(nnetFit, testingFin[,reducedSet],type='prob')[,'Y']
validation$pred.nnetFit <- predict(nnetFit, testingFin[,reducedSet])

# Model : Neural Network (model2)
validation$prob.nnetFit2 <- predict(nnetFit2, testingFin[,reducedSet],type='prob')[,'Y']
validation$pred.nnetFit2 <- predict(nnetFit2, testingFin[,reducedSet])

# Model : Neural Network (model3)
validation$prob.nnetFit3 <- predict(nnetFit3, testingFin[,reducedSet],type='prob')[,'Y']
validation$pred.nnetFit3 <- predict(nnetFit3, testingFin[,reducedSet])

# Model : Neural Network (model4)
validation$prob.nnetFit4 <- predict(nnetFit4, testingFin[,reducedSet],type='prob')[,'Y']
validation$pred.nnetFit4 <- predict(nnetFit4, testingFin[,reducedSet])

# Model : Penalized Logistic Regression 
validation$prob.glmnFit<- predict(glmnFit, testingFin[,reducedSet],type='prob')[,'Y']
validation$pred.glmnFit <- predict(glmnFit, testingFin[,reducedSet])

# Model : Support Vector Machine - Radial Kernel Basis
validation$prob.svmRFitFull <- predict(svmRFitFull, testingFin[,reducedSet],type='prob')[,'Y']
validation$pred.svmRFitFull <- predict(svmRFitFull, testingFin[,reducedSet])

# Model : Support Vector Machine - Polynomial Kernel Basis
validation$prob.svmPFitFull <- predict(svmPFitFull, testingFin[,reducedSet],type='prob')[,'Y']
validation$pred.svmPFitFull <- predict(svmPFitFull, testingFin[,reducedSet])


save(validation , file = paste('results/validation_allsampl',nsample,'.seed',123,'.rda',sep=''))
write.table(validation, paste('results/validation_allsample',nsample,'.seed',123,'.Rdata',sep=''), row.names = F, col.names = T, sep=',')

# print session info
sessionInfo()

# Append to the file
sink('results/analysis-output.txt', append=TRUE)
sink()
